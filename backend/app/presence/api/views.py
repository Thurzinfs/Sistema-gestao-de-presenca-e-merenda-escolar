...
def response_reading(request, id: UUID):
    ...
    return ReadingOut(reading)

@reading_router.get('/', response=ReadingOut)
...
def frequency_register(request, data: FrequencyIn):
    dto = FrequencyIn.to_dto()

    use_case = container.register_frequency_use_case()

    frequency = use_case.execute()
...
def register_snack_register(request, data: RegisterSnackIn):
    ...
    register_sanck = use_case.execute()

    return RegisterSnackOut.from_domain(register_sanck)
...
@register_snack_router.get('/{moment}', response=List[RegisterSnackOut])
...
@register_snack_router.get('/{type_snack}', response=List[RegisterSnackOut])
