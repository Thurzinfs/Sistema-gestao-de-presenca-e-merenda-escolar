...
class ReadingRegisterUseCase:
    ...
    def execute(self, dto: ReadingInDTO) -> ReadingOutDTO:
        if not self.reading_repo.very_exists_by_student_id(dto.student_id):
            raise Exception('Reading already')
...
class ReadingResponseUseCase:
    ...
    def execute(self, id: UUID) -> ReadingOutDTO:
        if self.reading_repo.very_exists(id):
...
class RegisterSnackRegisterUseCase:
    ...
    def execute(self, dto: RegisterSnackInDTO) -> RegisterSnackOut:
        if self.register_snack_repo.very_exist_register_snack_by_student_id(dto.student_id):
            raise Exception('Register snack already recorded')

        entity = RegisterSnackEntity(
            id=dto.id,
            stundet_id=dto.student_id,
            date=dto.date,
            moment=dto.moment,
        ...
        return RegisterSnackInDTO.from_domain(entity)
